...
import com.forlost.sunflower.ui.chatkit.dialogs.DialogsList;
...
import static com.forlost.sunflower.conf.Config.DIALOG_TYPE_P2P;
import static com.forlost.sunflower.conf.Config.DIALOG_TYPE_TRIBE;

/**
 * Adapter for {@link DialogsList}
 */
@SuppressWarnings("WeakerAccess")
public class MyDialogsListAdapter
        extends RecyclerView.Adapter<MyDialogsListAdapter.BaseDialogViewHolder> {
    ...
    private OnDialogLongClickListener onLongItemClickListener;
    private OnDialogViewLongClickListener onDialogViewLongClickListener;
     ...
     */
    public MyDialogsListAdapter(ImageLoader imageLoader) {
     ...
     * @param imageLoader  image loading method
     */
    public MyDialogsListAdapter(@LayoutRes int itemLayoutId, ImageLoader imageLoader) {
     ...
     * @param imageLoader  image loading method
     */
    public MyDialogsListAdapter(@LayoutRes int itemLayoutId, Class<? extends BaseDialogViewHolder> holderClass,
...
//            if (items.get(i).dialogId.equals(id)) {
//                items.remove(i);
//                notifyItemRemoved(i);
//            }
        }
    ...
    public void updateItemById(Dialog item) {
        ...
        for (int i = 0; i < items.size(); i++) {
//            if (items.get(i).dialogId.equals(item.dialogId)) {
//                items.set(i, item);
//                notifyItemChanged(i);
//                break;
//            }
...
//            if (item.dialogId == null && id == null) {
//                return item;
//            } else if (item.dialogId != null && item.dialogId.equals(id)) {
//                return item;
//            }
//        }
        return null;
     ...
     */
    @SuppressWarnings("unchecked")
    public boolean updateDialogWithMessage(String dialogId, Message message) {
        boolean dialogExist = false;
        for (int i = 0; i < items.size(); i++) {
//            if (items.get(i).dialogId.equals(dialogId)) {
//                items.get(i).lastLocalMessageId = message.id;
//                notifyItemChanged(i);
//                if (i != 0) {
    ...
    public void sortByLastMessageDate() {
        Collections.sort(items, new Comparator<Dialog>() {
            ...
            public int compare(Dialog o1, Dialog o2) {
                Message message1=o1.getLastMessage();
                Message message2=o2.getLastMessage();
...
//                if (message1.crateAt.after(message2.crateAt)) {
//                    return -1;
//                } else if (message1.crateAt.before(message2.crateAt)) {
//                    return 1;
//                } else return 0;
                return 0;
            }
     ...
     */
    public void setOnDialogViewClickListener(OnDialogViewClickListener clickListener) {
     ...
     */
    public OnDialogViewLongClickListener getOnDialogViewLongClickListener() {
     ...
     */
    public void setOnDialogViewLongClickListener(OnDialogViewLongClickListener clickListener) {
    ...
    public interface OnDialogClickListener {
        void onDialogClick(com.forlost.sunflower.room.Dialog dialog);
    }
    ...
    public interface OnDialogViewLongClickListener {
    ...
    public abstract static class BaseDialogViewHolder
        ...
        protected OnDialogViewLongClickListener onDialogViewLongClickListener;
    ...
    public static class DialogViewHolder extends BaseDialogViewHolder {
        ...
        public void onBind(final Dialog dialog) {
            ...
            //Set Name
            tvName.setText(dialog.dialogName);
            ...
            if (message != null && message.createAt != null) {
                Date lastMessageDate = message.createAt;
            ...
            //Set Dialog avatar
            if (imageLoader != null) {
                imageLoader.loadImage(ivAvatar, dialog.photo, null);
            }

            //Set Last message user avatar with check if there is last message
            if (imageLoader != null && user != null && user.avatar != null) {
                imageLoader.loadImage(ivLastMessageUser, user.avatar, null);
            }
            ivLastMessageUser.setVisibility(dialogStyle.isDialogMessageAvatarEnabled()
                    && dialog.dialogType == DIALOG_TYPE_TRIBE
                    && dialog.lastLocalMessageId > 0 ? VISIBLE : GONE);

            //Set Last message text
            if (dialog.lastLocalMessageId > 0 && message != null) {
                tvLastMessage.setText(message.getDialogViewText());
            } else {
                tvLastMessage.setText(null);
            }

            //Set Unread message count bubble
            tvBubble.setText(String.valueOf(dialog.unreadCount));
            tvBubble.setVisibility(dialogStyle.isDialogUnreadBubbleEnabled() &&
                    dialog.unreadCount > 0 ? VISIBLE : GONE);
